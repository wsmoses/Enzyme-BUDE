...
@eval Base.Threads function threading_run(func::F) where F
...
@eval Base.Threads function _threadsfor(iter, lbody, schedule)
...
@eval @fastmath function fasten_main(
  ...
  poses::Array{Float32,2},
  etotals::Array{Float32}, # Only output
) where {WGSIZE}
  nposes::Int = size(poses)[2]
  numGroups::Int = nposes ÷ WGSIZE

  Enzyme.@parallel protein ligand forcefield poses etotals for group = 1:numGroups
